...
exports.updateUploadImage = function(username, img){
    var query = { username: username };
    var partialUpdate = { $set: { currentUpload: img}};
    g.mongoCollection.update(query, partialUpdate, function checkError(error){
        if (error)
            throw error;
    });
    console.log('updated');
...
exports.getUserImages = function(username){
   var images = g.mongoCollection.find({username:username},{images:1});
   //console.log("got Images\n"+images.toArray());
   return images;
}

exports.addImage = function(username,img,title,desc){
    var query = {username: username};
    var partialUpdate = {$addToSet: {images:{file:img,title:title,desc:desc}}};
    g.mongoCollection.update(query,partialUpdate, function checkError(error){
        if(error)
            throw error;
    });
